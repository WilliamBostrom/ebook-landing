...
let questionCurrentyExplaind = $state(-1)

function onclick(index){
  if(questionCurrentyExplaind === index) {
    questionCurrentyExplaind = -1;
  } else{ 
  questionCurrentyExplaind = index;}
}
...
<section class="landing-page-section">
  ...
  <div class="faq-container">
    ...
    <FaqItem {faq} isExpanded={index === questionCurrentyExplaind} onclick={()=>onclick(index)}/>
    {/each}
  </div>
  <div class="additional-info mt-m">
    <Button class="mt-5">Buy Now</Button>
    <p class="mt-m">Any other qustions?</p>
    <a href="http://www.google.se">
    <p>Hit me up on google
